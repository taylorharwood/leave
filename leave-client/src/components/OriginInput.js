...
class Input extends Component {
  ...
  componentDidMount() {
    ...
    this.addAutoComplete(originInput).addBlurListeners(originInput);
  }

  addAutoComplete(input) {
    let originAutocomplete = new google.maps.places.Autocomplete(input);

    originAutocomplete.addListener('place_changed', () => {
      this.props.setOriginValue(input.value);
    });
  ...
  addBlurListeners(input) {
    ...
    google.maps.event.addDomListener(input, 'blur', event => {
      service.getPlacePredictions({ input: input.value }, (predictions, status) => {
        if (status === 'OK') {
          this.props.setOriginValue(predictions[0].description);
        }
      });
    });
